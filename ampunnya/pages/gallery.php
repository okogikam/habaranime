...
if(isset($_POST['delete_photo'])){
    $id = $_POST['photo_id'];
    $hasil = deletePhoto($id);
}else if(isset($_POST['publish'])){
    $id = $_POST['photo_id'];
    $hasil = displayPhoto($id);
}else{
    $hasil = "";
}
if(!empty($hasil)){
          ...
          <div class="row d-flex align-items-stretch">
              ...
              <div class='card bg-light'>
                ...
                <div class='card-body pt-0'>
                  <img class='img-fluid' src='$photo[gambar]' alt='$photo[alt]'>                   
                </div>
                <div class='card-footer'>
                  ...
                  <button type='submit' name='publish' class='btn btn-sm'>
                      <i class='$eye'></i>
                    </button>
                    <button type='submit' name='delete_photo' class='btn btn-sm bg-teal' onclick='return konfirmasi()'>
                      <i class='fas fa-trash'></i>
                    </button>
                    <a href='?page=Tambah%20Media&&id=$photo[no]' class='btn btn-sm btn-primary'>
